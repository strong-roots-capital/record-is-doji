...
describe('is-doji', () => {
    ...
    describe('#isDoji()', () => {
        ...
        it('should throw an error when given a spread less-than-or-equal-to 0', () => {
            expect(() => isDoji(record, 0)).to.throw()
            expect(() => isDoji(record, -1)).to.throw()
            expect(() => isDoji(record, 0.1)).to.not.throw()
        })

        it('should throw an error when given a spread greater-than 100', () => {
            expect(() => isDoji(record, 100)).to.not.throw()
            expect(() => isDoji(record, 100.1)).to.throw()
        })
